...
Create a virtual environment using python 3.9, and install EVE:
...
Replace `PYTHON_PATH` by your python path, e.g. `/usr/bin`, `X` by the specific python version you are using and `ENVIRONMENT_PATH` by the path to the virtual environments on your machine and follow the instructions below:
```bash
virtualenv -p PYTHON_PATH/python3.9.X ENVIRONMENT_PATH/Eve
pip install eve-SMLM"
```
